...
class LTCCell(nn.Module):
    ...
    def _ode_solver(self, inputs, state, elapsed_time):
        ...
        sensory_w_activation = self._params["sensory_w"] * sensory_matrix
        sensory_mask = self._params["sensory_sparsity_mask"].to(
            sensory_w_activation.device
        )
        sensory_w_activation *= sensory_mask

        sensory_rev_activation = self._params["sensory_erev"] * sensory_matrix
        sensory_rev_activation *= sensory_mask
        ...
        for t in range(self._ode_unfolds):
            ...
            w_activation = self._params["w"] * state_matrix
            state_mask = self._params["sparsity_mask"].to(w_activation.device)
            w_activation *= state_mask

            rev_activation = self._params["erev"]*state_matrix
            rev_activation *= state_mask
