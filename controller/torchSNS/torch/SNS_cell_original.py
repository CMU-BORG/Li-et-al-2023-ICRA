...
class SNSCell_Original(nn.Module):
    def __init__(
        self,
        wiring,
        in_features=None,
        ...
        super(SNSCell_Original, self).__init__()
    ...
    def _ode_solver(self, inputs, state, elapsed_time):
        ...
        sensory_rev_activation = self._params["sensory_erev"] * self._params["sensory_w"] * sensory_activation
        ...
        for t in range(self._ode_unfolds):
            ...
            state_rev_activation = self._params["erev"] * self._params["w"] * state_activation
            ...
            sum_w = torch.sum(state_w_activation, dim=1) + sensory_w + 1

            num = v_pre*self._params["tau"]/delta + sum_rev
            den = self._params["tau"]/delta + sum_w
            v_pre = num/den
    ...
    def apply_weight_constraints(self):
        self._params["tau"].data = self._clipl(self._params["tau"].data)
        self._params["w"].data = self._clipl(self._params["w"].data)
        self._params["sensory_w"].data = self._clipl(self._params["sensory_w"].data)
        #self._params["erev"].data = self._cliph(self._params["erev"].data,190*self._params["w"].data)
        #self._params["sensory_erev"].data = self._cliph(self._params["sensory_erev"].data,190*self._params["sensory_w"].data)
