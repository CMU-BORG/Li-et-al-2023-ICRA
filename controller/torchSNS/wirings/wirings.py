...
class TwoLayers(Wiring):
    def __init__(
        self,
        hidden_neurons,
        motor_neurons,
        recurrent_hidden_synapses,
        seed=22222,
    ):

        super(TwoLayers, self).__init__(hidden_neurons + motor_neurons)
        self.set_output_dim(motor_neurons)
        self._rng = np.random.RandomState(seed)
        self._num_hidden_neurons = hidden_neurons
        self._num_motor_neurons = motor_neurons
        self._recurrent_command_synapses = recurrent_hidden_synapses

        self._motor_neurons = list(range(self._num_hidden_neurons, self._num_hidden_neurons + self._num_motor_neurons))
        self._hidden_neurons = list(
            range(
                0,
                self._num_hidden_neurons
            )
        )

    def get_type_of_neuron(self, neuron_id):
        if neuron_id < self._num_hidden_neurons:
            return "hidden"
        return "motor"

    def _build_sensory_to_hidden_layer(self):
        if self._num_hidden_neurons == self._num_sensory_neurons:
            for src in self._sensory_neurons:
                polarity = self._rng.choice([-1, 1])
                self.add_sensory_synapse(src, src, polarity)
        else:
            for src in self._sensory_neurons:
                for dest in self._hidden_neurons:
                    polarity = self._rng.choice([-1, 1])
                    self.add_sensory_synapse(src, dest, polarity)

    def _build_hidden_to_motor_layer(self):
        for src in self._hidden_neurons:
            for dest in self._motor_neurons:
                polarity = self._rng.choice([-1, 1])
                self.add_synapse(src, dest, polarity)
    ...
    def build(self, input_shape):
        ...
        self._build_sensory_to_hidden_layer()
        self._build_hidden_to_motor_layer()
        if self._recurrent_command_synapses is True:
            self._build_recurrent_command_layer()